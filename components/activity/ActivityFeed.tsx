...
const TYPE_CONFIG: Record<string, { icon: React.ElementType; color: string; label: (d: any) => string }> = {
  ...
  duel_created:  { icon: Swords,        color: 'text-violet-400',  label: d => `Yeni bir düello başlattı` },
  ...
  level_up:      { icon: Zap,           color: 'text-cyan-400',    label: d => `Seviye atladı` },
}
...
export default function ActivityFeed({ username, limit = 20 }: Props) {
  ...
  return (
    <div className="space-y-1">
      {items.map((item, idx) => {
        const cfg = TYPE_CONFIG[item.type] ?? { icon: Zap, color: 'text-white/40', label: () => item.type }
        const Icon = cfg.icon
        ...
        return (
          <div
            key={item.id}
            className="flex items-start gap-3 px-1 py-2.5 hover:bg-white/3 rounded-xl transition-colors"
          >
            {/* Icon bubble */}
            <div className={`w-8 h-8 rounded-full bg-white/5 flex items-center justify-center shrink-0 ${cfg.color}`}>
              <Icon size={14} />
            </div>
            ...
            {/* Time */}
            <span className="text-[10px] text-white/25 shrink-0 mt-0.5">{timeAgo(item.created_at)}</span>
          </div>
        )
      })}
    </div>
