...
export default function ReferralCard({ userId }: Props) {
  ...
  function copyLink() {
    ...
    if (navigator.share) {
      navigator.share({
        title: 'Kapisio — Düello Platformu',
        text: 'Seninle düelloya girmeye geldim! Davet kodum:',
        url: link,
      }).catch(() => {})
    } else {
  ...
  if (loading) return (
    <div className="rounded-2xl bg-[#1a1a2e] border border-white/10 p-4 animate-pulse h-24" />
  )

  return (
    <div className="rounded-2xl bg-[#1a1a2e] border border-green-500/15 overflow-hidden">
      {/* Header */}
      <div className="px-4 pt-4 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Gift size={14} className="text-green-400" />
          <span className="text-sm font-bold text-white">Arkadaşını Davet Et</span>
        </div>
        {count > 0 && (
        ...
        {code && (
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2">
              <p className="text-[10px] text-white/30 mb-0.5">Davet Kodun</p>
              <p className="font-mono font-black text-white tracking-widest text-sm">{code.toUpperCase()}</p>
            </div>
            <button
              onClick={copyLink}
              className="flex items-center gap-1.5 bg-green-600 hover:bg-green-500 text-white rounded-xl px-3 py-2 text-xs font-semibold transition-colors h-full"
            >
        ...
        <div className="flex items-center justify-between text-xs text-white/30">
          ...
          <div className="flex items-center gap-1">
            <div className="flex gap-0.5">
              {[1,2,3,4,5].map(i => (
                <div key={i} className={`w-4 h-1.5 rounded-full ${i <= count ? 'bg-green-500' : 'bg-white/10'}`} />
              ))}
            </div>
